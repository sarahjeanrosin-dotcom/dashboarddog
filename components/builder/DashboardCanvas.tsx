...
export default function DashboardCanvas() {
  ...
  function getWidgetAreaSize() {
    ...
    return { width: el.offsetWidth, height: el.scrollHeight / heightMultiplier }
  }
      ...
      >
        ...
        >
          ...
          <div className="space-y-[4%]">
            <p className="text-[0.8cqw] font-semibold uppercase tracking-widest text-white opacity-60">{selectedVertical?.name}</p>
            <p className="text-[1.6cqw] font-bold leading-tight text-white">{selectedRole?.avatar_emoji} {selectedRole?.title}</p>
            {selectedRole?.blurb && <p className="text-[0.85cqw] text-white opacity-70 leading-snug">{selectedRole.blurb}</p>}
          </div>
        ...
        >
          {frameUrl ? (
            <div style={{ position: 'relative', width: '100%', height: `${heightMultiplier * 100}%`, minHeight: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              <div className="flex items-center gap-[1.5%] px-[2%] shrink-0 bg-white border-b border-gray-200" style={{ height: '10%', position: 'sticky', top: 0, zIndex: 10 }}>
                ...
                <div className="ml-auto flex items-center gap-[1.5%]">
                  ...
                  <div className="flex items-center gap-[0.5%]">
                    <div className="rounded-full flex items-center justify-center text-[0.7cqw] font-bold text-white" style={{ width: '2cqw', height: '2cqw', backgroundColor: primaryColor }}>{selectedRole?.avatar_emoji ?? 'U'}</div>
                    <ChevronDown style={{ width: '1cqw', height: '1cqw' }} className="text-gray-400" />
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', flex: 1 }}>
                <div className="flex flex-col py-[2%] bg-white border-r border-gray-200" style={{ width: '12%', position: 'sticky', top: '10%', alignSelf: 'flex-start' }}>
                  {PRODUCT_NAV.map(({ icon: Icon, label }, i) => (
                    <div key={label} className="flex items-center gap-[8%] px-[12%] py-[6%] mx-[8%] rounded cursor-pointer" style={i === 0 ? { backgroundColor: primaryColor + '18', color: primaryColor } : { color: '#6b7280' }}>
                      <Icon style={{ width: '1.2cqw', height: '1.2cqw', flexShrink: 0 }} />
                      <span className="text-[0.75cqw] font-medium truncate">{label}</span>
                    </div>
                  ))}
                </div>
                <div ref={widgetAreaRef} className="relative flex-1 bg-[#f0f2f5] p-[1.5%]" style={{ height: `${heightMultiplier * 90}%`, minHeight: '90%', cursor: dragging || resizing ? 'grabbing' : 'default' }}>
                  {renderWidgets({ layout, widgets, dragging, resizing, accentColor, heightMultiplier, onWidgetMouseDown, onResizeMouseDown })}
                </div>
              </div>
            </div>
...
function renderWidgets({ layout, widgets, dragging, resizing, accentColor, heightMultiplier, onWidgetMouseDown, onResizeMouseDown }: {
  layout: WidgetLayout[]
  ...
  return layout.map(item => {
    ...
    return (
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-black/50 text-white px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity truncate" style={{ fontSize: '0.65cqw' }}>{widget.name}</div>
        <div className="absolute bottom-0 right-0 opacity-0 group-hover:opacity-100 cursor-se-resize" style={{ width: '1.2cqw', height: '1.2cqw', backgroundColor: accentColor }} onMouseDown={e => onResizeMouseDown(e, item.widget_id)} />
      </div>
    )
  })
}
