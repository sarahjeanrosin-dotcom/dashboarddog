...
export default function MaskEditor({ widget, onSave, onCancel }: Props) {
  ...
  async function handleAiDetect() {
    ...
    try {
      ...
      // Expand boxes by a small padding to ensure full coverage of original text
      const PAD = 3
        ...
        {image ? (
          ...
          >
            <Layer>
              ...
              />
              {masks.map(mask => {
                // Font size: fit text snugly within the box height
                const fontSize = Math.max(9, Math.round(mask.height * 0.55))
                return mask.replacementText ? (
                  // AI replacement: white background rect + anonymized text
                  <>
                    <Rect
                      key={`${mask.id}-bg`}
                      id={mask.id}
